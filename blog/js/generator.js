...
<feed xmlns="http://www.w3.org/2005/Atom" xmlns:media="http://search.yahoo.com/mrss/">
		...
		<id>https://jxpx777.me/blog/</id>
		<icon>https://jxpx777.me/favicon.ico</icon>
		<logo>https://jxpx777.me/apple-touch-icon.png</logo>
		<link rel="alternate" href="https://jxpx777.me/blog/"/>
		<link rel="self" href="https://jxpx777.me/blog/feed.xml"/>
		<updated>%UPDATED%</updated>
		<author>
				<name>Jamie Phelps</name>
		</author>
%ENTRIES%
</feed>`;
...
customElements.define(
  ...
  class BlogGenerator extends HTMLElement {
    ...
    addClickListener() {
      this.addEventListener("click", () => {
        ...
        if (!window.showDirectoryPicker) {
          this.showError(
            "Opening folders with a picker is not supported in your browser.",
          );
        } else {
          window
            .showDirectoryPicker({
        ...
        try {
          ...
          if (item.kind === "file") {
            if (!item.getAsFileSystemHandle) {
              throw new Error(
                "Dropping files is not supported in your browser.",
              );
            }
    ...
    async startProcessing(blogFolder) {
      ...
      if (
        this.#blogFolder.kind !== "directory" ||
        this.#blogFolder.name !== "blog"
      ) {
        this.#blogFolder = null;
        ...
        if (value.kind === "directory") {
          ...
          try {
            ...
            if (e.name === "NotFoundError") {
              this.addMessage(
                `Folder ${key}/ does not have an index.html file, skipping.`,
                "warning",
              );
              continue;
            }
            throw new Error(
              "Error processing " + value.name + ": " + e.message,
            );
          }
        }
      }
    ...
    async processArticle(article, path) {
      ...
      const published = dom.querySelector("article").dataset["published"];
      const content = await this.processArticleContent(
        dom.querySelector("main article"),
        path,
      );
      const slug = path.name;
      // optional
      const img = dom.querySelector("main article img");
      const image = img && {
    ...
    async processArticleContent(main, path) {
      ...
      [...main.querySelectorAll("img")].map((elem) => {
        ...
        if (src.indexOf("http") !== 0) {
          elem.setAttribute(
            "src",
            new URL(`articles/${path.name}/${src}`, BLOG_BASE_URL),
          );
        }
      });
        ...
        this.#articles
          ...
          .map((a) => {
            ...
            const author = a.author
              ? `<author><name><![CDATA[${a.author}]]></name></author>`
              : "";
            return `    <entry>
